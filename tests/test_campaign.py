...
class CampaignTests(unittest.TestCase):
    ...
    def test_victory_publishes_once(self) -> None:
        ...
        steps = [
            (["fist"], 2.15),
            (["point"], 3.25),
            (["open_palm", "open_palm"], 4.35),
            (["open_palm"], 5.45),
            (["fist"], 6.55),
        ]
        event = None
        for gestures, now in steps:
            event = engine.update(gestures, now, lambda pill: calls.append(pill) or True)
        ...
        repeat = engine.update(["open_palm", "open_palm"], 6.55, lambda pill: calls.append(pill) or True)
